...
import { Zap, ArrowLeft, Plus, Trash2, AlertTriangle, GripVertical, ArrowRight, Sparkles, Loader2, Save } from "lucide-react";
...
const EditHiringLink = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (error || !link) {
        toast({ title: "Not found", description: "Hiring link not found.", variant: "destructive" });
        navigate("/dashboard");
        return;
      }
      ...
      setTimeLimitMinutes(link.time_limit_minutes);
      setMaxTabSwitches(link.max_tab_switches);
      setPassingScore(link.passing_score);

      const { data: qs } = await supabase
        .from("questions")
        .select("*")
        .eq("hiring_link_id", linkId)
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("generate-mcqs", {
        body: { jobTitle: title.trim(), jobDescription: description.trim() || undefined, numberOfQuestions: aiCount },
      });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await supabase.from("questions").delete().eq("hiring_link_id", linkId);

      const questionsToInsert = questions.map((q, i) => ({
        hiring_link_id: linkId,
      ...
      toast({ title: "Saved!", description: "Hiring link updated successfully." });
      navigate("/dashboard");
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container max-w-3xl py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <h1 className="text-3xl font-display font-bold text-foreground mb-2">Edit Hiring Link</h1>
          <p className="text-muted-foreground mb-8">Update your job details, settings, and questions.</p>
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                    <div className="flex items-center gap-2 flex-1">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">Re-generate questions based on job title. This replaces existing questions.</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Input type="number" min={1} max={20} value={aiCount} onChange={(e) => setAiCount(Number(e.target.value))} className="w-16 bg-background border-border h-9 text-center" />
                      <Button type="button" size="sm" onClick={handleGenerateAI} disabled={aiLoading} className="gap-1.5">
                  ...
                  <CardContent className="pt-6 space-y-4">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2">
                          <Switch checked={q.is_knockout} onCheckedChange={(v) => updateQuestion(q.id, "is_knockout", v)} />
                    ...
                    <Input value={q.question_text} onChange={(e) => updateQuestion(q.id, "question_text", e.target.value)} placeholder="Enter your question..." className="bg-background border-border" required />
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {q.options.map((opt, optIdx) => (
                        <div key={optIdx} className="flex items-center gap-2">
                          ...
                          <Input value={opt} onChange={(e) => updateOption(q.id, optIdx, e.target.value)} placeholder={`Option ${String.fromCharCode(65 + optIdx)}`} className="bg-background border-border" required />
                        </div>
                      ))}
                    </div>
